...
/// Карта сдвигается вправо при нажатии на кнопку, но только если масштаб увеличен.
/// </summary>
public class Right : MonoBehaviour
{
    ...
    [Tooltip("Насколько перемещать карту вправо за одно нажатие (в единицах)")]
    ...
    [Tooltip("Кнопка перемещения вправо (→)")]
    ...
    /// Вызывается при нажатии на кнопку вправо.
    ...
    public void MoveMapRight()
    {
        ...
        // Устанавливаем целевую позицию на moveAmount правее
    ...
    /// Вызывается при нажатии на кнопку влево.
    /// </summary>
    ...
    private void SetupUI()
    {
        if (rightButton != null)
        {
            Button button = rightButton.GetComponent<Button>();
            if (button == null)
            {
                Debug.LogWarning($"[{GetType().Name}] Добавьте компонент Button на {rightButton.name}!", this);
                return;
            }

            button.onClick.RemoveAllListeners();
            button.onClick.AddListener(MoveMapRight);
        }
        else
        {
            Debug.LogWarning($"[{GetType().Name}] rightButton не назначен в Inspector!", this);
        }
    ...
    private void OnGUI()
    {
        ...
        GUILayout.BeginArea(new Rect(300, 160, 280, 180));
        GUILayout.BeginVertical("box");
        GUILayout.Label("═══════════════════════════════");
        GUILayout.Label("➡️ Right Map Debug");
        GUILayout.Label("═══════════════════════════════");
