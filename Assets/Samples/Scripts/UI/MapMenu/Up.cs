...
/// Карта поднимается вверх при нажатии на кнопку, но только если масштаб увеличен.
/// </summary>
public class Up : MonoBehaviour
{
    ...
    [Tooltip("Насколько перемещать карту вверх за одно нажатие (в единицах)")]
    ...
    [Tooltip("Кнопка перемещения вверх (↑)")]
    ...
    public void MoveMapUp()
    {
        ...
        // Устанавливаем целевую позицию на moveAmount выше
    ...
    private void SetupUI()
    {
        if (upButton != null)
        {
            Button button = upButton.GetComponent<Button>();
            if (button == null)
            {
                Debug.LogWarning($"[{GetType().Name}] Добавьте компонент Button на {upButton.name}!", this);
                return;
            }

            button.onClick.RemoveAllListeners();
            button.onClick.AddListener(MoveMapUp);
        }
        else
        {
            Debug.LogWarning($"[{GetType().Name}] upButton не назначен в Inspector!", this);
        }
    ...
    private void OnGUI()
    {
        ...
        GUILayout.BeginArea(new Rect(10, 160, 280, 180));
        GUILayout.BeginVertical("box");
        GUILayout.Label("═══════════════════════════════");
        GUILayout.Label("⬆️ Up Map Debug");
        GUILayout.Label("═══════════════════════════════");
