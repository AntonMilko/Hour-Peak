...
/// Карта сдвигается влево при нажатии на кнопку, но только если масштаб увеличен.
/// </summary>
public class Left : MonoBehaviour
{
    ...
    [Tooltip("Насколько перемещать карту влево за одно нажатие (в единицах)")]
    ...
    [Tooltip("Кнопка перемещения влево (←)")]
    ...
    public void MoveMapLeft()
    {
        ...
        // Устанавливаем целевую позицию на moveAmount левее
    ...
    private void SetupUI()
    {
        if (leftButton != null)
        {
            Button button = leftButton.GetComponent<Button>();
            if (button == null)
            {
                Debug.LogWarning($"[{GetType().Name}] Добавьте компонент Button на {leftButton.name}!", this);
                return;
            }

            button.onClick.RemoveAllListeners();
            button.onClick.AddListener(MoveMapLeft);
        }
        else
        {
            Debug.LogWarning($"[{GetType().Name}] leftButton не назначен в Inspector!", this);
        }
    ...
    private void OnGUI()
    {
        ...
        GUILayout.BeginArea(new Rect(10, 520, 280, 180));
        GUILayout.BeginVertical("box");
        GUILayout.Label("═══════════════════════════════");
        GUILayout.Label("⬅️ Left Map Debug");
        GUILayout.Label("═══════════════════════════════");
